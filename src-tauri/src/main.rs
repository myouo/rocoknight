...
fn change_channel(app: AppHandle, state: State<Mutex<AppState>>) -> Result<(), String> {
    let (has_projector, has_swf) =
        with_state(&state, |s| (s.projector.is_some(), s.swf_url.is_some()));
    if !has_projector {
...
#[tauri::command]
fn get_speed(state: State<Mutex<AppState>>) -> f64 {
    with_state(&state, |s| s.speed_multiplier)
}

#[tauri::command]
fn set_speed(_app: AppHandle, state: State<Mutex<AppState>>, multiplier: f64) -> f64 {
    let clamped = if multiplier < 0.1 {
        0.1
    ...
    with_state(&state, |s| {
        ...
        if let Some(ref shmem) = s.speed_shmem {
            shmem.set_multiplier(clamped);
        }
    });
    info!("[RocoKnight][speed] speed set to {}x", clamped);
    clamped
}
...
fn main() {
    ...
    let app_result = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Ok(projector_path) = app.path().resolve("resources/projector.exe", BaseDirectory::Resource)
            {
